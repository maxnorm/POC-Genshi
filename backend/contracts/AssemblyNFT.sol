...
contract AssemblyNFT is ERC998TopDown, Documents, Attributes, AccessManaged, ChildContractValidator, TokenAuthorized {
  ...
  constructor(address templateRegistry, address accessManager, address[] memory allowedChildrenContracts) 
    ERC998TopDown("AssemblyNFT", "ASSEMBLY") 
  ...
  function mint(address to, uint256 templateId) external override
    onlyRole(accessManager.ASSEMBLY_MINTER())
    returns (uint256)
  {
  ...
  {
    require(
      accessManager.hasRole(accessManager.ASSEMBLY_DOCUMENT_MANAGER(), msg.sender) ||
      accessManager.hasRole(accessManager.ASSEMBLY_MANAGER(), msg.sender),
      Access_NotAuthorized(accessManager.ASSEMBLY_DOCUMENT_MANAGER())
    );
