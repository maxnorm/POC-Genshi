...
contract TemplateRegistry is AccessManaged {
  ...
  function createTemplate(
    ...
    newTemplate.allowedValidators = validators;
  ...
  function addAttribute(uint256 templateId, string memory key, ITemplate.AttributeDefinition memory attribute) external onlyRole(accessManager.ADMIN_ROLE()) {
  ...
  function addDocument(uint256 templateId, string memory key, ITemplate.DocumentDefinition memory document) external onlyRole(accessManager.ADMIN_ROLE()) {
    require(templates[templateId].id <= _count, Template_InvalidTemplate(templateId));
  ...
  function activateTemplate(uint256 templateId) external onlyRole(accessManager.ADMIN_ROLE()) {
    require(templates[templateId].id <= _count, Template_InvalidTemplate(templateId));
  ...
  /// @param key The key of the attribute to validate
  /// @param value The value of the attribute to validate
  function _validateEnum(ITemplate.AttributeDefinition memory attribute, string memory key, string memory value) internal pure returns (bool) {
